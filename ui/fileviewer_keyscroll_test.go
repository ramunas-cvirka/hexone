...
func testPDFDocViewerState(pages int) *fileViewerState {
	st := &fileViewerState{
		mode:                  "file",
	...
	st.pdfDoc.configure(viewerPDFDocInfo{PageCount: pages, PageSizes: sizes})
	return st
}

func TestPerformFileViewerKeyScrollPDFDownArrowScrollsCombinedDocument(t *testing.T) {
	ui := NewUI(fm.DefaultConfig())
	now := time.Date(2026, time.April, 11, 15, 0, 0, 0, time.UTC)
	st := testPDFDocViewerState(3)
	ui.fileViewer = st

	if !ui.performFileViewerKeyScroll(now, key.NameDownArrow) {
		t.Fatal("Down should scroll the PDF document")
	}
	if st.pdfDoc.scrollY <= 0 {
		t.Fatalf("scrollY=%f want > 0", st.pdfDoc.scrollY)
	}
}

func TestPerformFileViewerKeyScrollPDFCrossesPageBoundary(t *testing.T) {
	ui := NewUI(fm.DefaultConfig())
	now := time.Date(2026, time.April, 11, 15, 0, 0, 0, time.UTC)
	st := testPDFDocViewerState(3)
	ui.fileViewer = st

	// Scroll down until the current page changes; combined scrolling must
	// cross the page boundary without any page-flip render.
	for i := 0; i < 100 && st.pdfDoc.currentPage() == 0; i++ {
		if !ui.performFileViewerKeyScroll(now, key.NamePageDown) {
			break
		}
	}
	if got := st.pdfDoc.currentPage(); got == 0 {
		t.Fatal("PageDown should eventually reach the next page in the combined document")
	}
}

func TestPerformFileViewerKeyScrollPDFEndJumpsToDocumentEnd(t *testing.T) {
	ui := NewUI(fm.DefaultConfig())
	now := time.Date(2026, time.April, 11, 15, 0, 0, 0, time.UTC)
	st := testPDFDocViewerState(3)
	ui.fileViewer = st
	...
	if !ui.performFileViewerKeyScroll(now, key.NameEnd) {
		t.Fatal("End should scroll to the end of the document")
	}
	_, maxY := st.pdfDoc.maxScroll()
	if got := st.pdfDoc.scrollY; got != maxY {
		t.Fatalf("scrollY=%f want %f (combined document end)", got, maxY)
	}
	if got := st.pdfDoc.currentPage(); got != 2 {
		t.Fatalf("currentPage=%d want 2", got)
	}
	if !ui.performFileViewerKeyScroll(now, key.NameHome) {
		t.Fatal("Home should scroll back to the start")
	}
	if got := st.pdfDoc.scrollY; got != 0 {
		t.Fatalf("scrollY=%f want 0", got)
	}
}

func TestPDFDocWheelScrollsAcrossPages(t *testing.T) {
	st := testPDFDocViewerState(2)
	v := &st.pdfDoc

	if !v.scrollWheelY(3) {
		t.Fatal("wheel should scroll the document")
	}
	if v.scrollY <= 0 {
		t.Fatalf("scrollY=%f want > 0", v.scrollY)
	}
	before := v.scrollY
	for i := 0; i < 10000 && v.scrollWheelY(3); i++ {
	}
	_, maxY := v.maxScroll()
	if v.scrollY != maxY {
		t.Fatalf("scrollY=%f want %f after exhausting wheel scroll", v.scrollY, maxY)
	}
	if v.scrollY <= before {
		t.Fatal("wheel scrolling should have crossed into the second page")
	}
}
