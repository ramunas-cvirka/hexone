...
func (ui *UI) layoutFilePanes(th *material.Theme, gtx layout.Context) layout.Dimensions {
	...
	return layout.Stack{}.Layout(gtx,
		...
		layout.Stacked(func(gtx layout.Context) layout.Dimensions {
			return ui.layoutFilePaneVolumeBadges(th, gtx, visible)
		}),
	)
}
...
func (ui *UI) layoutFilePaneVolumeBadges(th *material.Theme, gtx layout.Context, visible []visiblePane) layout.Dimensions {
	...
	for i, item := range visible {
		paneWidth := widths[i]
		if paneWidth <= 0 || item.pane == nil {
			x += paneWidth
			continue
		}
		paneRect := image.Rect(x, 0, x+paneWidth, gtx.Constraints.Max.Y)
		x += paneWidth

		m := op.Record(gtx.Ops)
		paneGtx := gtx
		paneGtx.Constraints = layout.Constraints{Max: paneRect.Size()}
		ui.layoutFilePaneVolumeBadge(th, paneGtx, item.idx, item.pane, item.idx == ui.activeFilePane, filePanePaletteFromConfig(ui.fmCfg))
		call := m.Stop()

		stack := op.Offset(paneRect.Min).Push(gtx.Ops)
		call.Add(gtx.Ops)
		stack.Pop()
	}
	return layout.Dimensions{Size: gtx.Constraints.Max}
					...
					return layout.W.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
						return layout.Inset{Left: unit.Dp(2)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
							...
							return layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}.Layout(gtx,
								...
								layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
									return ui.layoutFilePanePathFill(th, gtx, pane, fillH)
								}),
							)
						})
					})
...
func (ui *UI) layoutFilePanePathFill(th *material.Theme, gtx layout.Context, pane *filePaneState, fillH int) layout.Dimensions {
	w := gtx.Constraints.Max.X
	if w <= 0 || fillH <= 0 {
		return layout.Dimensions{Size: image.Pt(w, fillH)}
	}
	palette := filePanePaletteFromConfig(ui.fmCfg)
	return fixedHeight(gtx, fillH, func(gtx layout.Context) layout.Dimensions {
		return layoutFilePaneFrameLine(gtx, w, filePanePathFrameColor(palette))
	})
}
...
func filePanePathFrameColor(palette filePanePalette) color.NRGBA {
	out := mixNRGBA(palette.CurrentDirFg, palette.CurrentDirBg, 0.48)
	out.A = 170
	return out
}

func filePanePathIndicatorColor(palette filePanePalette) color.NRGBA {
	luma := 0.2126*float64(palette.CurrentDirBg.R) + 0.7152*float64(palette.CurrentDirBg.G) + 0.0722*float64(palette.CurrentDirBg.B)
	if luma >= 150 {
		return color.NRGBA{R: 0, G: 102, B: 138, A: 255}
	}
	return color.NRGBA{R: 92, G: 214, B: 255, A: 255}
}
...
func (ui *UI) filePaneHeaderTextStyle(pane *filePaneState) (font.Typeface, unit.Sp) {
	return ui.currentDirTypeface(), ui.currentDirTextSize()
}

func (ui *UI) filePaneHeaderStripHeight(gtx layout.Context, pane *filePaneState) int {
	_, textSize := ui.filePaneHeaderTextStyle(pane)
	h := gtx.Sp(textSize) + gtx.Dp(unit.Dp(7))
...
func (ui *UI) filePaneHeaderIconSize(gtx layout.Context, pane *filePaneState) int {
	_, textSize := ui.filePaneHeaderTextStyle(pane)
	size := gtx.Sp(textSize)
	if size < gtx.Dp(unit.Dp(12)) {
		size = gtx.Dp(unit.Dp(12))
	}
	return size
}

func (ui *UI) layoutFilePanePathSegmentLabel(th *material.Theme, gtx layout.Context, pane *filePaneState, label string, bg, fg, border color.NRGBA, weight font.Weight) layout.Dimensions {
...
func (ui *UI) layoutFilePanePath(th *material.Theme, gtx layout.Context, idx int, pane *filePaneState, active bool) layout.Dimensions {
	...
	palette := filePanePaletteFromConfig(ui.fmCfg)
	if pane.filterClick.Clicked(gtx) {
		ui.setActiveFilePane(idx)
		pane.closeSortMenu()
		pane.closeDriveMenu()
		pane.closeFavoriteMenu()
	...
	if pane.remoteConnected() {
		...
		children := ui.filePaneBreadcrumbStart(th, pane, palette, len(segments)*2+3)
		for i := range segments {
			i := i
			if i > 0 {
				children = append(children, ui.filePaneBreadcrumbSeparator(th, pane, palette, "›"))
			}
			children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				if segments[i].path == "" {
				...
				return click.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
					...
					return ui.layoutFilePanePathSegmentLabel(th, gtx, pane, segments[i].label, bg, lblColor, border, weight)
				})
			}))
		}
		children = ui.filePaneBreadcrumbEnd(th, palette, idx, pane, children)
...
func (ui *UI) filePaneBreadcrumbStart(th *material.Theme, pane *filePaneState, palette filePanePalette, capacity int) []layout.FlexChild {
	children := make([]layout.FlexChild, 0, capacity+1)
	children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
		return layoutFilePaneFrameLine(gtx, ui.filePaneFrameEdgeWidth(th, gtx, pane), filePanePathFrameColor(palette))
	}))
	return append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
		return ui.layoutFilePaneFrameBracket(gtx, pane, true, filePanePathFrameColor(palette))
	}))
}
...
func (ui *UI) filePaneBreadcrumbEnd(th *material.Theme, palette filePanePalette, idx int, pane *filePaneState, children []layout.FlexChild) []layout.FlexChild {
	children = append(children, ui.filePaneBreadcrumbSeparator(th, pane, palette, ">"))
	children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
		return ui.layoutFilePaneFilter(th, gtx, idx, pane, palette)
	}))
	children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
...
func (ui *UI) layoutFilePaneFilter(th *material.Theme, gtx layout.Context, idx int, pane *filePaneState, palette filePanePalette) layout.Dimensions {
	if pane == nil {
		return layout.Dimensions{}
	}
	return pane.filterClick.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
		pointer.CursorText.Add(gtx.Ops)
		maxW := gtx.Dp(unit.Dp(80))
		if gtx.Constraints.Max.X > maxW {
			gtx.Constraints.Max.X = maxW
		}
		bg := color.NRGBA{}
		fg := filePanePathBaseColor(palette)
		border := color.NRGBA{}
		if pane.filterClick.Hovered() {
			bg, fg, border = filePanePathHoverColors(palette)
		}
		weight := font.Normal
		if pane.displayFilter() != filePaneDefaultFilter {
			weight = font.Medium
		}
		return layoutLeftInsetPx(gtx, ui.filePaneFilterLeadingInset(gtx, pane), func(gtx layout.Context) layout.Dimensions {
			return ui.layoutFilePanePathSegmentLabel(th, gtx, pane, pane.displayFilter(), bg, fg, border, weight)
		})
	})
}

func (ui *UI) filePaneFilterLeadingInset(gtx layout.Context, pane *filePaneState) int {
	_, textSize := ui.filePaneHeaderTextStyle(pane)
	em := max(1, gtx.Sp(textSize))
	return max(1, (em*2+5)/11)
}

func layoutLeftInsetPx(gtx layout.Context, left int, child layout.Widget) layout.Dimensions {
	if left <= 0 {
		return child(gtx)
	}
	constraints := gtx.Constraints
	constraints.Max.X -= left
	...
	offset := op.Offset(image.Pt(left, 0)).Push(gtx.Ops)
	dims := child(gtx)
	offset.Pop()
	dims.Size.X += left
	return dims
...
func (ui *UI) layoutFilePanePathEditor(th *material.Theme, gtx layout.Context, idx int, pane *filePaneState, active bool) layout.Dimensions {
	...
	ed.Font.Typeface, ed.TextSize = ui.filePaneHeaderTextStyle(pane)
	palette := filePanePaletteFromConfig(ui.fmCfg)
...
func filePaneSortMenuBaseRect(gtx layout.Context, pane *filePaneState, size image.Point, slideY int) image.Rectangle {
	right := gtx.Constraints.Max.X - pane.sortControlRightInset
	if pane.sortControlWidth <= 0 {
		const favoriteBadgeAreaDp = 30
		right = gtx.Constraints.Max.X - gtx.Dp(unit.Dp(favoriteBadgeAreaDp))
	}
	anchor := image.Point{
		X: right - size.X,
...
func (ui *UI) layoutFilePaneControlStrip(th *material.Theme, gtx layout.Context, idx int, pane *filePaneState) layout.Dimensions {
	...
	palette := filePanePaletteFromConfig(ui.fmCfg)
	return fixedHeight(gtx, stripH, func(gtx layout.Context) layout.Dimensions {
		var sortW, separatorAfterSortW, favoriteW, remoteSeparatorW, disconnectW, closeW int
		...
		children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			return ui.layoutFilePaneFrameSeparator(gtx, pane, filePanePathIndicatorColor(palette))
		}))
		children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			dims := pane.sortClick.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				pointer.CursorPointer.Add(gtx.Ops)
				bg := color.NRGBA{}
				fg := txtColor
				if pane.sortMenuOpen {
				...
				return fillBgExact(gtx, bg, func(gtx layout.Context) layout.Dimensions {
					return layout.Inset{Top: unit.Dp(2), Bottom: unit.Dp(2)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
						return layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}.Layout(gtx,
							layout.Rigid(func(gtx layout.Context) layout.Dimensions {
								typeface, textSize := ui.filePaneHeaderTextStyle(pane)
								lbl := material.Body2(th, pane.sortKey.badgeLabel())
								lbl.Font.Typeface = typeface
								lbl.Font.Weight = font.Medium
								lbl.TextSize = textSize
								lbl.Color = fg
								lbl.MaxLines = 1
								return layoutInkVCenteredLabel(gtx, lbl)
							}),
							layout.Rigid(func(gtx layout.Context) layout.Dimensions {
								_, textSize := ui.filePaneHeaderTextStyle(pane)
								return layoutFilePaneSortDirectionArrow(gtx, textSize, pane.sortDesc, fg)
							}),
						)
					})
				})
		...
		children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			dims := ui.layoutFilePaneFrameSeparator(gtx, pane, filePanePathIndicatorColor(palette))
			separatorAfterSortW = dims.Size.X
			return dims
		}))
		children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			dims := pane.favoriteClick.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				pointer.CursorPointer.Add(gtx.Ops)
				bg := color.NRGBA{}
				fg := txtColor
				if pane.favoriteMenuOpen {
				...
				return fillBgExact(gtx, bg, func(gtx layout.Context) layout.Dimensions {
					return layout.Inset{Top: unit.Dp(2), Bottom: unit.Dp(2)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
						size := ui.filePaneHeaderIconSize(gtx, pane) - 1
						if size < 1 {
							size = 1
						}
						return layout.Center.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
			...
			favoriteW = dims.Size.X
			return dims
		}))
		if pane.remoteConnected() {
			children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				dims := ui.layoutFilePaneFrameSeparator(gtx, pane, filePanePathIndicatorColor(palette))
				remoteSeparatorW = dims.Size.X
				return dims
			}))
			children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				dims := pane.disconnectClick.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
					pointer.CursorPointer.Add(gtx.Ops)
					bg := color.NRGBA{}
					iconColor := txtColor
		...
		trailingW := remoteSeparatorW + disconnectW + closeW
		pane.favoriteControlWidth = favoriteW
		pane.favoriteControlRightInset = trailingW
		pane.sortControlWidth = sortW
		pane.sortControlRightInset = separatorAfterSortW + favoriteW + trailingW
		return dims
	})
}
...
func layoutFilePaneFrameBracket(gtx layout.Context, textSize unit.Sp, opening bool, fg color.NRGBA) layout.Dimensions {
	...
	if maxH := gtx.Constraints.Max.Y; maxH > 0 && h > maxH {
		h = maxH
	}
	if w < 1 || h < 1 {
		return layout.Dimensions{}
	...
	return layout.Center.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
		...
		inset := stroke / 2
		top := inset
		bottom := float32(h-1) - inset
		left := inset
		right := float32(w-1) - inset
		verticalX := left
		armX := right
		if !opening {
			verticalX, armX = right, left
		}
		var p clip.Path
		p.Begin(gtx.Ops)
		p.MoveTo(f32.Pt(verticalX, top))
		p.LineTo(f32.Pt(verticalX, bottom))
		p.MoveTo(f32.Pt(verticalX, top))
		p.LineTo(f32.Pt(armX, top))
		p.MoveTo(f32.Pt(verticalX, bottom))
		p.LineTo(f32.Pt(armX, bottom))
		paint.FillShape(gtx.Ops, fg, clip.Stroke{Path: p.End(), Width: stroke}.Op())
		return layout.Dimensions{Size: size}
	})
}

func (ui *UI) layoutFilePaneFrameSeparator(gtx layout.Context, pane *filePaneState, fg color.NRGBA) layout.Dimensions {
	_, textSize := ui.filePaneHeaderTextStyle(pane)
	em := max(1, gtx.Sp(textSize))
	w := max(5, (em*6+10)/11)
	h := max(8, em)
	...
	return layout.Center.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
		...
		var p clip.Path
		p.Begin(gtx.Ops)
		p.MoveTo(f32.Pt(x, inset))
		p.LineTo(f32.Pt(x, float32(h-1)-inset))
		paint.FillShape(gtx.Ops, fg, clip.Stroke{Path: p.End(), Width: stroke}.Op())
		return layout.Dimensions{Size: size}
	})
}

func (ui *UI) filePaneFrameEdgeWidth(th *material.Theme, gtx layout.Context, pane *filePaneState) int {
	typeface, textSize := ui.filePaneHeaderTextStyle(pane)
	...
	probe.MaxLines = 1
	w := measureLabelUnconstrained(gtx, probe).Size.X
	if w < 1 {
		w = 1
	}
	return w
...
func layoutFilePaneFrameLine(gtx layout.Context, width int, fg color.NRGBA) layout.Dimensions {
	if width < 1 {
		return layout.Dimensions{}
	}
...
func layoutFilePaneSortDirectionArrow(gtx layout.Context, textSize unit.Sp, descending bool, fg color.NRGBA) layout.Dimensions {
	em := gtx.Sp(textSize)
	if em < 1 {
		em = 1
	}
	...
	size := image.Pt(w, h)
	return layout.Center.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
		centerX := float32(w-1) / 2
		tipY := float32(1)
		tailY := float32(h - 1)
		armY := min(float32(h-1), tipY+float32(max(2, h/3)))
		if descending {
			tipY, tailY = tailY, tipY
			armY = max(float32(0), tipY-float32(max(2, h/3)))
		}
		armX := float32(max(2, w/3))
		var p clip.Path
		p.Begin(gtx.Ops)
		p.MoveTo(f32.Pt(centerX, tailY))
		p.LineTo(f32.Pt(centerX, tipY))
		p.MoveTo(f32.Pt(centerX-armX, armY))
		p.LineTo(f32.Pt(centerX, tipY))
		p.LineTo(f32.Pt(centerX+armX, armY))
		stroke := float32(em) / 9
		if stroke < 1 {
			stroke = 1
		}
...
func (ui *UI) layoutFilePaneFavoriteMenu(th *material.Theme, gtx layout.Context, idx int, pane *filePaneState) layout.Dimensions {
	...
	for {
		...
		inOrderMenu := pane.favoriteOrderOpen && filePaneFavoriteRectContains(pane.favoriteOrderRect, pos)
		if pe.Buttons.Contain(pointer.ButtonSecondary) {
			if itemIndex, ok := ui.filePaneFavoriteSavedItemAt(gtx, menuRect, items, pos); ok {
				pane.openFavoriteOrderMenu(items[itemIndex].configIndex, pos)
				gtx.Execute(op.InvalidateCmd{})
				continue
			}
			pane.closeFavoriteOrderMenu()
			if !inMenu {
				pane.closeFavoriteMenu()
			}
			gtx.Execute(op.InvalidateCmd{})
		...
		if inOrderMenu {
			if step, ok := ui.filePaneFavoriteOrderStepAt(gtx, pane, pos); ok {
				moved, err := ui.moveFavoriteLocation(pane.favoriteOrderIndex, step)
				if err != nil {
					pane.setNotice("failed to save favorites: "+err.Error(), gtx.Now)
				}
				if moved {
		...
		if pane.favoriteOrderOpen && !inOrderMenu {
			pane.closeFavoriteOrderMenu()
			gtx.Execute(op.InvalidateCmd{})
		}
		overFavoriteToggle := pane.favoriteClick.Pressed() || pane.favoriteClick.Hovered()
		if !inMenu && !inOrderMenu && !overFavoriteToggle {
			pane.closeFavoriteMenu()
			gtx.Execute(op.InvalidateCmd{})
		}
	}
	...
	ui.layoutFilePaneFavoriteOrderMenu(th, gtx, pane, alpha)
	bodyClip.Pop()
...
func (ui *UI) filePaneFavoriteSavedItemAt(gtx layout.Context, menuRect image.Rectangle, items []fileFavoriteItem, pos image.Point) (int, bool) {
	if !filePaneFavoriteRectContains(menuRect, pos) {
		return -1, false
	}
	rowH := ui.filePaneFavoriteMenuRowHeight(gtx)
	for i, item := range items {
		if !item.removable || item.configIndex < 0 {
			continue
		}
		y := menuRect.Min.Y + ui.filePaneFavoriteMenuItemOffsetY(gtx, items, i)
...
func (ui *UI) filePaneFavoriteOrderStepAt(gtx layout.Context, pane *filePaneState, pos image.Point) (int, bool) {
	if ui == nil || pane == nil || !pane.favoriteOrderOpen || ui.fmCfg == nil || !filePaneFavoriteRectContains(pane.favoriteOrderRect, pos) {
		return 0, false
	}
	items := filePaneFavoriteOrderItems(pane.favoriteOrderIndex, len(ui.fmCfg.FavoriteLocations))
	y := pane.favoriteOrderRect.Min.Y
	for i, item := range items {
		rowH := ui.fileContextMenuRowHeight(gtx, item)
		if pos.Y >= y && pos.Y < y+rowH {
			if item.Disabled {
...
func (ui *UI) layoutFilePaneFavoriteOrderMenu(th *material.Theme, gtx layout.Context, pane *filePaneState, alpha float32) layout.Dimensions {
	if ui == nil || pane == nil || !pane.favoriteOrderOpen || ui.fmCfg == nil {
		return layout.Dimensions{}
	}
	for i := range pane.favoriteOrderClicks {
		for pane.favoriteOrderClicks[i].Clicked(gtx) {
		}
	}
	index := pane.favoriteOrderIndex
	items := filePaneFavoriteOrderItems(index, len(ui.fmCfg.FavoriteLocations))
	spec := fileContextMenuSpec{Key: "favorite-order", WidthDp: filePaneContextMenuCompactWidthDp, Items: items}
	size := ui.fileContextMenuPanelSize(th, gtx, spec)
	anchor := clampFilePaneMenuPoint(pane.favoriteOrderPos.Add(image.Pt(2, 2)), size, gtx.Constraints.Max)
	pane.favoriteOrderRect = image.Rectangle{Min: anchor, Max: anchor.Add(size)}

	theme := ui.filePanePopupTheme()
	childGTX := gtx
	childGTX.Constraints = layout.Exact(size)
	offset := op.Offset(anchor).Push(gtx.Ops)
	dims := fixedWidth(childGTX, size.X, func(gtx layout.Context) layout.Dimensions {
		return fillRoundedClipBox(
			gtx,
			gtx.Dp(unit.Dp(filePaneOverlayCornerDp)),
			scaleColorAlpha(theme.Bg, alpha),
			...
			func(gtx layout.Context) layout.Dimensions {
				...
				for i, item := range items {
					i, item := i, item
					rowH := ui.fileContextMenuRowHeight(gtx, item)
					rowRect := image.Rect(anchor.X, y, anchor.X+size.X, y+rowH)
					y += rowH
					children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						hoverFill := float32(0)
						if pane.favoritePointerPosSet && filePaneFavoriteRectContains(rowRect, pane.favoritePointerPos) {
							hoverFill = 1
						}
						dims, _, _ := ui.layoutFilePaneContextMenuItem(th, gtx, theme, &pane.favoriteOrderClicks[i], item, false, hoverFill, alpha, rowH)
						return dims
					}))
				}
				return layout.Flex{Axis: layout.Vertical}.Layout(gtx, children...)
...
func filePaneFavoriteOrderItems(index, count int) []fileContextMenuItem {
	return []fileContextMenuItem{
		{ID: "favorite-move-up", Label: "Move up", Disabled: index <= 0},
		{ID: "favorite-move-down", Label: "Move down", Disabled: index < 0 || index >= count-1},
	}
}
...
func (ui *UI) favoriteMenuRevealHotspotRect(th *material.Theme, gtx layout.Context, menuRect image.Rectangle, items []fileFavoriteItem, index int, item fileFavoriteItem) image.Rectangle {
	...
	maxWidth := ui.filePaneFavoriteMenuTextWidth(gtx, item)
	...
	rowTop := menuRect.Min.Y + ui.filePaneFavoriteMenuItemOffsetY(gtx, items, index)
	rowH := ui.filePaneFavoriteMenuRowHeight(gtx)
...
func (ui *UI) favoriteMenuRevealRect(th *material.Theme, gtx layout.Context, pane *filePaneState, menuRect image.Rectangle, items []fileFavoriteItem) image.Rectangle {
	...
	rowTop := menuRect.Min.Y + ui.filePaneFavoriteMenuItemOffsetY(gtx, items, revealIndex)
	rowH := ui.filePaneFavoriteMenuRowHeight(gtx)
...
func (ui *UI) filePaneFavoriteMenuWidth(gtx layout.Context) int {
	width := gtx.Dp(ui.scaleInterfaceDp(unit.Dp(filePaneFavoriteMenuWidthDp)))
	if width > gtx.Constraints.Max.X {
		width = gtx.Constraints.Max.X
	}
	if width < 1 {
...
func (ui *UI) filePaneFavoriteMenuTitleHeight(gtx layout.Context) int {
	titleH := gtx.Sp(ui.scaleInterfaceFontSize(9)) + gtx.Dp(ui.scaleInterfaceDp(6))
	if titleH < 1 {
		titleH = 1
	}
	return titleH
...
func (ui *UI) filePaneFavoriteMenuRowHeight(gtx layout.Context) int {
	rowH := gtx.Sp(ui.functionBarTextSize()) + gtx.Dp(ui.scaleInterfaceDp(7))
	if rowH < 1 {
		rowH = 1
	}
	return rowH
...
func (ui *UI) filePaneFavoriteMenuSeparatorHeight(gtx layout.Context) int {
	sepH := gtx.Dp(ui.scaleInterfaceDp(5))
	if sepH < 3 {
		sepH = 3
	}
	return sepH
...
func (ui *UI) filePaneFavoriteMenuCardSize(gtx layout.Context, items []fileFavoriteItem) image.Point {
	height := ui.filePaneFavoriteMenuTitleHeight(gtx)
	for i, item := range items {
		if item.addCurrent && i > 0 {
			height += ui.filePaneFavoriteMenuSeparatorHeight(gtx)
		}
		height += ui.filePaneFavoriteMenuRowHeight(gtx)
	}
	return image.Pt(ui.filePaneFavoriteMenuWidth(gtx), height)
}

func (ui *UI) filePaneFavoriteRemoveButtonWidth(gtx layout.Context) int {
	size := gtx.Dp(ui.scaleInterfaceDp(10))
	if size < 1 {
		size = 1
	}
	return size + gtx.Dp(ui.scaleInterfaceDp(6))
}
...
func (ui *UI) filePaneFavoriteMenuBaseRect(gtx layout.Context, pane *filePaneState, items []fileFavoriteItem, slideY int) image.Rectangle {
	...
	right := gtx.Constraints.Max.X - pane.favoriteControlRightInset
	if pane.favoriteControlWidth <= 0 {
		right = gtx.Constraints.Max.X
	}
	anchor := image.Point{
		X: right - size.X,
		Y: gtx.Dp(unit.Dp(tabStripHeightDp)) + pane.headerHeight + gtx.Dp(unit.Dp(2)) + slideY,
...
func (ui *UI) favoriteMenuLabelMetrics(th *material.Theme, gtx layout.Context, item fileFavoriteItem) (fullWidth, trimmedWidth, hiddenPrefixWidth, ellipsisWidth int) {
	...
	textWidth := ui.filePaneFavoriteMenuTextWidth(gtx, item)
	if textWidth <= 0 {
...
func (ui *UI) filePaneFavoriteMenuItemOffsetY(gtx layout.Context, items []fileFavoriteItem, target int) int {
	y := ui.filePaneFavoriteMenuTitleHeight(gtx)
	for i, item := range items {
		if i >= target {
			break
		}
		if item.addCurrent && i > 0 {
			y += ui.filePaneFavoriteMenuSeparatorHeight(gtx)
		}
		y += ui.filePaneFavoriteMenuRowHeight(gtx)
	}
		...
		if item.removable && !item.disabled {
			...
			children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				if interactive && removeClick != nil {
					return ui.layoutFilePaneFavoriteRemoveButton(th, gtx, theme, removeClick, alpha)
				}
				return ui.layoutFilePaneFavoriteRemoveButtonVisual(gtx, theme, alpha, false)
			}))
		}
		return layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}.Layout(gtx, children...)
	})
		...
		return pane.favoriteMenuClick.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
			return fillRoundedClipBox(
				...
				func(gtx layout.Context) layout.Dimensions {
					...
					children = append(children, layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						titleH := ui.filePaneFavoriteMenuTitleHeight(gtx)
						return fixedHeight(gtx, titleH, func(gtx layout.Context) layout.Dimensions {
							return layout.Inset{Left: unit.Dp(7), Right: unit.Dp(7), Top: unit.Dp(3), Bottom: unit.Dp(1)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
...
func (ui *UI) layoutFilePaneFavoriteRemoveButtonVisual(gtx layout.Context, theme filePanePopupTheme, alpha float32, hovered bool) layout.Dimensions {
	bg := color.NRGBA{}
	iconColor := scaleColorAlpha(bestContrastColor(theme.Bg, theme.Text, theme.HoverText, theme.ActiveText), alpha*0.72)
	if hovered {
		bg = scaleColorAlpha(color.NRGBA{R: 112, G: 40, B: 52, A: 238}, alpha)
		iconColor = scaleColorAlpha(color.NRGBA{R: 255, G: 150, B: 164, A: 255}, alpha)
	}
	buttonW := ui.filePaneFavoriteRemoveButtonWidth(gtx)
...
func (ui *UI) layoutFilePaneFavoriteRemoveButton(_ *material.Theme, gtx layout.Context, theme filePanePopupTheme, c *widget.Clickable, alpha float32) layout.Dimensions {
	return c.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
		pointer.CursorPointer.Add(gtx.Ops)
		return ui.layoutFilePaneFavoriteRemoveButtonVisual(gtx, theme, alpha, c.Hovered())
	})
}
