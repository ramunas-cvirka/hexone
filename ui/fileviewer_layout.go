...
func (ui *UI) layoutFileViewerHeader(th *material.Theme, gtx layout.Context, st *fileViewerState) layout.Dimensions {
	...
	if st.loading && st.content == "" && (status == "" || status == "loading...") {
		status = "loading..."
	}
	summary := title + " | " + status
	if !st.updatedAt.IsZero() {
		summary += " | pulled " + st.updatedAt.Format("15:04:05")
	}
		...
		func(gtx layout.Context) layout.Dimensions {
			return layout.Inset{Left: unit.Dp(8), Right: unit.Dp(8), Top: unit.Dp(2), Bottom: unit.Dp(2)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
				row := layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					summaryLbl := material.Body2(th, summary)
					summaryLbl.Font.Typeface = ui.mainTypeface()
					summaryLbl.Font.Weight = font.Medium
					summaryLbl.TextSize = scaleThemeFontSize(th, 10)
					summaryLbl.Color = color.NRGBA{R: 210, G: 224, B: 250, A: 255}
					summaryLbl.MaxLines = 1
					summaryLbl.Truncator = "..."

					summaryW := measureLabelUnconstrained(gtx, summaryLbl).Size.X
					if summaryW < 1 {
						summaryW = 1
					}
					reserve := gtx.Dp(unit.Dp(190))
					if st.mode == "command" {
						reserve = gtx.Dp(unit.Dp(320))
					}
					maxSummaryW := gtx.Constraints.Max.X - reserve
					if maxSummaryW < gtx.Dp(unit.Dp(120)) {
						maxSummaryW = gtx.Dp(unit.Dp(120))
					}
					if summaryW > maxSummaryW {
						summaryW = maxSummaryW
					}

					summaryChild := layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						return fixedWidth(gtx, summaryW, summaryLbl.Layout)
					})
					sepChild := layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						sep := material.Body2(th, "|")
						sep.Font.Typeface = ui.mainTypeface()
						sep.TextSize = scaleThemeFontSize(th, 10)
						sep.Color = color.NRGBA{R: 182, G: 196, B: 224, A: 255}
						return sep.Layout(gtx)
					})
					tabsChild := layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						return ui.layoutFileViewerModeTabs(th, gtx, st)
					})
					...
					return layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}.Layout(gtx,
						...
						layout.Rigid(layout.Spacer{Width: unit.Dp(4)}.Layout),
						layout.Rigid(func(gtx layout.Context) layout.Dimensions {
							return layoutTinyIconModeButton(th, gtx, &st.autoRefreshClick, uiRefreshGlyphIcon(), st.autoRefresh)
						}),
						layout.Rigid(layout.Spacer{Width: unit.Dp(4)}.Layout),
						closeChild,
					)
...
func (ui *UI) layoutFileViewerModeTabs(th *material.Theme, gtx layout.Context, st *fileViewerState) layout.Dimensions {
	if st == nil {
		return layout.Dimensions{}
	}
	return fillRoundedBox(
		...
		func(gtx layout.Context) layout.Dimensions {
			return layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}.Layout(gtx,
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					return ui.layoutFileViewerModeTabSegment(th, gtx, &st.modeFileClick, "File", st.mode == "file")
				}),
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					w := gtx.Dp(unit.Dp(1))
					if w < 1 {
					...
					return fillBgExact(gtx, color.NRGBA{R: 255, G: 255, B: 255, A: 24}, func(gtx layout.Context) layout.Dimensions {
						return layout.Dimensions{Size: image.Pt(w, gtx.Dp(unit.Dp(22)))}
					})
				}),
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					return ui.layoutFileViewerModeTabSegment(th, gtx, &st.modeCmdClick, "Cmd", st.mode == "command")
				}),
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					w := gtx.Dp(unit.Dp(1))
					if w < 1 {
				...
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					return ui.layoutFileViewerModeTabSegment(th, gtx, &st.historyClick, "..", st.historyOpen)
				}),
			)
		},
	)
...
func (ui *UI) layoutFileViewerModeTabSegment(th *material.Theme, gtx layout.Context, c *widget.Clickable, label string, active bool) layout.Dimensions {
	if c == nil {
		return layout.Dimensions{}
	}
...
func (ui *UI) layoutFileViewerHistoryListRows(th *material.Theme, gtx layout.Context, st *fileViewerState, history []string) layout.Dimensions {
	if len(history) == 0 {
		return layout.Inset{Left: unit.Dp(6), Right: unit.Dp(6), Top: unit.Dp(5), Bottom: unit.Dp(5)}.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
