...
func gio_onCommandBySelector(h C.uintptr_t) C.bool {
	...
	for _, k := range ev.eventStr {
		...
		if !ok {
			// Function keys can trigger doCommandBySelector on macOS even though
			// the corresponding key event has already been dispatched from keyDown.
			// Claim them here to prevent AppKit fallback beeps.
			if n, fn := convertKey(k); fn && isFunctionKeyName(n) {
				handled = true
			}
			continue
		}
...
func isFunctionKeyName(name key.Name) bool {
	switch name {
	case key.NameF1, key.NameF2, key.NameF3, key.NameF4, key.NameF5, key.NameF6, key.NameF7, key.NameF8, key.NameF9, key.NameF10, key.NameF11, key.NameF12:
		return true
	default:
		return false
	}
